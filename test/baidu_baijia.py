...
#t.get(format=output_format,etl_count=100)
...
kr.keep('publish_time title cover desc content url')
...
st.keep('timestamp:publish_time,html:content,cover,desc,url') #date:timestamp
...
si.keep('intime:publish_time,intro:desc,title,cover,Content:content,url') #keywords
st.pyft('publish_time',sc=time_script,stop_while=True)
si.replace('desc,content',sc=u'新浪娱乐讯',new_value='')

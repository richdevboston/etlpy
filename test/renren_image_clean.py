...
b=task('pic')
b.dbge('mongo',table='renren_album').rm('_id').let('url').map(get_header).nullft().cp('id:fo').map('value[:2]').format('/gruntdata/desert.zym/dev/renren_large/{0}/{id}/').let('').map(copy).fetch(10)
